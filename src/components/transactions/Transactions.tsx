...
const Transactions = () => {
  ...
  const [changeDate, setchangeDate] = useState("27 Feb -- 06 Mar");
  ...
  const handleClickOutside = (event: any) => {
    if (wrapperRef.current && !wrapperRef.current.contains(event.target)) {
      if (
        ...
        event.target.className === "icon-calender"
      ) {
      } else {
        setDrop(false);
      }
  ...
  const handleSpan = (e: any) => {
    ...
    console.log("date", date, day, month);
  setchangeDate(e.target.innerText);
    switch (e.target.innerText) {
      case "Today":
        setchangeDate(`March,${day}th`);

      case "Yesterday":
        setchangeDate(`March,${day - 1}th`);
    }
    ...
    <div className="dashboard__box">
      ...
      <div className="transaction__box_content">
        <div className="transaction__top__box">
          ...
          <button className="date-btn" onClick={() => setDrop(!drop)}>
            {changeDate}
          </button>
          <span
            className="icon-filter-span"
